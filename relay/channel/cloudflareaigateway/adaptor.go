...
func (a *Adaptor) GetRequestURL(info *relaycommon.RelayInfo) (string, error) {
	...
	case constant.RelayModeChatCompletions:
		return fmt.Sprintf("%s/v1/%s/compat/chat/completions", info.ChannelBaseUrl, info.ApiVersion), nil
		// return fmt.Sprintf("%s/client/v4/accounts/%s/ai/v1/chat/completions", info.ChannelBaseUrl, info.ApiVersion), nil
	case constant.RelayModeGemini:
		return fmt.Sprintf("%s/v1/%s/google-ai-studio%s", info.ChannelBaseUrl, info.ApiVersion, info.RequestURLPath), nil
